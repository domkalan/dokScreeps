...
export function runTower(tower: StructureTower, context: RoomContext) {
    const visual = new RoomVisual(tower.room.name).text(`${tower.store.getUsedCapacity(RESOURCE_ENERGY) / tower.store.getCapacity(RESOURCE_ENERGY) * 100}%`, tower.pos.x, tower.pos.y + 1, { color: 'white', font: 0.5 });
    ...
    if (tower.store.getUsedCapacity(RESOURCE_ENERGY) / tower.store.getCapacity(RESOURCE_ENERGY) >= 0.75) {
        const damagedRegularStructures = context.structures.filter(structure => structure.hits < structure.hitsMax * 0.5 && structure.structureType !== STRUCTURE_WALL && structure.structureType !== STRUCTURE_RAMPART);
        const damagedDefenseStructures = context.structures.filter(structure => structure.hits < structure.hitsMax * 0.25 && (structure.structureType === STRUCTURE_RAMPART || structure.structureType === STRUCTURE_WALL));
