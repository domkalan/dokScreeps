...
const mainObf = async () => {
    const mainBuild = await readFileAsync(path.resolve('./dist/obf/main.js'))
    ...
    await writeFileAsync(path.resolve('./dist/obf/main.orig.js'), mainBuild);
    await writeFileAsync(path.resolve('./dist/obf/main.js'), newBuild);
};
